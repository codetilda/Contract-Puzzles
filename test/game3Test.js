...
describe('Game3', function () {
  async function deployContractAndSetVariables() {
    ...
    return { game, signer, address1, address2, address3, signer1, signer2, signer3 };
  }

  it('should be a winner', async function () {
    const { game, signer, address1, address2, address3, signer1, signer2, signer3 } = await loadFixture(deployContractAndSetVariables);

    // you'll need to update the `balances` mapping to win this stage
    await game.connect(signer1).buy({ value: '6' });
    await game.connect(signer2).buy({ value: '7' });
